...
import { checkPaymentStatus, createPaymentOrder, PAYMENT_AMOUNT } from "@/lib/mobilipa.functions";
...
function PaymentPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      const account = loadAccount();
      const updated = { ...account, phone };
      saveAccount(updated);
      const order = await createOrder({ data: { name: updated.fullName, email: updated.email, phone } });
      ...
      timer.current = setInterval(async () => {
        ...
        try {
          ...
          if (["COMPLETED", "SUCCESS", "SUCCESSFUL", "PAID"].includes(res.payment_status)) {
            if (timer.current) clearInterval(timer.current);
            saveAccount({ ...loadAccount(), phone, activated: true });
            navigate({ to: "/dashboard" });
            return;
          }
          if (["CANCELLED", "USERCANCELLED", "REJECTED", "FAILED"].includes(res.payment_status)) {
            if (timer.current) clearInterval(timer.current);
            setPhase("failed");
